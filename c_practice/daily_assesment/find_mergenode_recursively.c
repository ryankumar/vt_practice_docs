...
struct node *find_node(struct node *head1,struct node *head2)
{
	...
	if(cur1==cur2)
		...
		if(cur2)
			find_node(head1,cur2->next);
		else if(cur1)
		{
			if(cur2==NULL)
				cur2=hptr2;
			find_node(cur1->next,cur2);
		}
...
int main()
{
	...
	while(count<7)
	{
		...
		// TEST CASES6: if head2 is NULL or if no merging point
		// complecity= zero; 
		else if(count==5)
		{	
			ret=find_node(head1,head2);
		...
		// TEST CASES7: if head1 is NULL or if no merging point
		// complexity= zero;
		else if(count==6)
		{
			ret=find_node(head1,head2);
